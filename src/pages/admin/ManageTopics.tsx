  ...
  const handleActionSubmit = async () => {
    ...
    try {
      if (modalType === 'add') {
        ...
        await fetch(`http://localhost:8080/api/topics/subject/${subId}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ topicName: inputValue }) 
        });
        ...
        await fetch(`http://localhost:8080/api/topics/delete/${selectedTopicId}`, {
          method: 'DELETE'
        });
      }
            ...
            {topics.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {topics.map((topic, index) => {
                  const displayId = topic.id || topic.topicId || topic.topicID;
              ...
              {(modalType === 'rename' || modalType === 'delete') && (
                <div className="space-y-2">
                  ...
                  >
                    ...
                    {topics.map(t => {
                      const tId = t.id || t.topicId || t.topicID;
                      return (
